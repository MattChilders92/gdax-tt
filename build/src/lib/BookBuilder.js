...
class BookBuilder extends events_1.EventEmitter {
    ...
    getNOrders(side, n) {
        ...
        const iter = tree.findIter({ price: first.price, totalSize: first.totalSize });
        if (!iter) {
            return [];
        }
     ...
     */
    ordersForValue(side, limit = -1, value = types_1.Big(1000), useQuote, start) {
    ...
    getGroupedOrders(side, grouping = 0, limit = -1, value = types_1.Big(1000)) {
    ...
    getNodesAround(side, limit = 13, grouping = .01) {
        const source = side === 'buy' ? this.asks : this.bids;
